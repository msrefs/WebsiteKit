  ...
  return (
    <div
      ...
      <div
        ...
        } flex flex-col justify-between w-full min-h-[10rem] h-full md:h-fit max-w-xl dark:bg-hexo-black-gray dark:border-gray-800 bg-white dark:bg- p-5 rounded-full z-50 shadow border hover:border-emerald-400 duration-300 transition-all `}>
        <div className='flex justify-between items-center'>
          <div className='text-2xl text-emerald-400 dark:text-emerald-600 font-bold'>
            搜索
          </div>
          <div>
            <i
              className='text-gray-600 fa-solid fa-xmark p-1 cursor-pointer hover:text-emerald-400'
        ...
        <ul className='flex-1 overflow-auto'>
          {searchResults.map((result, index) => (
            <li
              ...
              rounded-full
              ${activeIndex === index ? 'bg-emerald-400 dark:bg-emerald-600' : ''}`}>
              <a
      ...
      {firstTenTags?.map((tag, index) => {
        return (
          <Link
            ...
            <div
              className={
                'flex items-center text-black dark:text-gray-300 hover:bg-emerald-400 dark:hover:bg-emerald-600 hover:scale-110 hover:text-white rounded-full px-2 py-0.5 duration-150 transition-all'
              }>
  ...
  return (
    <div className='flex space-x-1 w-full justify-center py-1'>
      {Array.from({ length: totalPage }, (_, i) => {
        const classNames =
          page === i
            ? 'font-bold text-white bg-emerald-400 dark:bg-emerald-600 rounded'
            : 'hover:text-emerald-400 hover:font-bold dark:text-gray-300'

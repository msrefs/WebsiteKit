      ...
      <div
        ...
        <div
          className={
            ...
            ' flex p-4 md:p-6 flex-col justify-between h-48 md:h-full w-full md:w-7/12 -mx-4 md:mx-0'
          }>
          {/* 内容部分完全保持不变 */}
          <header>
            {post?.category && (
              <div className={`flex mb-1 items-center ${showPreview ? 'justify-center' : 'justify-start'} hidden md:block flex-wrap dark:text-gray-300 text-gray-600 hover:text-emerald-400 dark:hover:text-emerald-400`}>
                <Link passHref href={`/category/${post.category}`} className='cursor-pointer text-xs font-normal menu-link'>
                  {post.category}
                </Link>
              </div>
            )}
            <Link href={post?.href} passHref className='group-hover:text-emerald-400 dark:hover:text-emerald-400 dark:group-hover:text-emerald-400 text-black dark:text-gray-100 line-clamp-2 replace cursor-pointer text-xl font-extrabold leading-tight'>
